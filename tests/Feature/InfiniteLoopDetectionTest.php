...
use Salette\PaginationPlugin\Exceptions\PaginationException;
...
use Salette\PaginationPlugin\Tests\Fixtures\Connectors\DisabledInfiniteLoopConnector;
...
test('the paginator only keeps five response checksums in memory at once', function () {
    ...
    expect($previousBodyChecksums)
        ->toBeArray()
        ->and($previousBodyChecksums)->toHaveCount(4)
        ->and($previousBodyChecksums[0])->toBe(md5(json_encode($responseD)))
        ->and($previousBodyChecksums[1])->toBe(md5(json_encode($responseE)))
        ->and($previousBodyChecksums[2])->toBe(md5(json_encode($responseF)))
        ->and($previousBodyChecksums[3])->toBe(md5(json_encode($responseG)));
...
test('you can disable the infinite loop detection on a paginator', function () {
    ...
    try {
        iterator_to_array($paginator);
    } catch (NoMockResponseFoundException $ex) {
