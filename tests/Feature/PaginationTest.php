...
use Salette\PaginationPlugin\Tests\Fixtures\Requests\UserRequest;
use Salette\PaginationPlugin\Tests\Fixtures\Connectors\PagedConnector;
use Salette\PaginationPlugin\Tests\Fixtures\Connectors\CursorConnector;
use Salette\PaginationPlugin\Tests\Fixtures\Connectors\OffsetConnector;
use Salette\PaginationPlugin\Tests\Fixtures\Requests\MappedPagedRequest;
use Salette\PaginationPlugin\Tests\Fixtures\Requests\SuperheroPagedRequest;
use Salette\PaginationPlugin\Tests\Fixtures\Requests\SuperheroCursorRequest;
use Salette\PaginationPlugin\Tests\Fixtures\Requests\SuperheroLimitOffsetRequest;
...
test('you can paginate automatically through many pages of results with paged pagination', function () {
    ...
    expect($collection)
        ...
        ->and($collection->pluck('id')->toArray())->toEqual(
...
test('you can specify the maximum number of pages to iterate over', function () {
    ...
    expect($iteratorCounter)
        ->toEqual(2)
        ->and($superheroes)->toHaveCount(10);
...
test('the paginator will throw an exception if you use a request that is not paginatable', function () {
    ...
    $this->expectExceptionMessage('The request must implement the `Salette\PaginationPlugin\Contracts\Paginatable` interface to be used on paginators.');
